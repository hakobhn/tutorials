...
* Interface com.core.example.methods by default;
* Lambda expressions;
* Functional com.core.example.interfaces;
* References to com.core.example.methods and com.core.example.constructors;
* Repeatable annotations
* Annotations on data types;
* Reflection for method parameters;
* Stream API for working with com.core.example.collections;
* Parallel sorting of com.core.example.arrays;
...
Java 8 interface changes include static com.core.example.methods and default com.core.example.methods in com.core.example.interfaces. Prior to Java 8, we could have only method declarations in the com.core.example.interfaces. But from Java 8, we can have default com.core.example.methods and static com.core.example.methods in the com.core.example.interfaces.
...
The default com.core.example.methods of the implemented functional interface are not allowed to be accessed inside the lambda expression.

<div align="right">
    <b><a href="#">↥ back to top</a></b>
</div>

## Q. How to sort a list of com.core.example.strings using a lambda expression?
...
An interface can include as many `default` com.core.example.methods as you like while remaining functional, because `default` com.core.example.methods are not abstract.
...
The class is StringJoinerused to create a sequence of com.core.example.strings separated by a separator with the ability to append a prefix and suffix to the resulting string:
```java
...
## Q. What are `default`interface com.core.example.methods?
...
```
* If a class implements an interface, it can, but does not have to, implement the default com.core.example.methods already implemented in the * interface. The class inherits the default implementation.
* If a class implements several com.core.example.interfaces that have the same default method, then the class must implement the method with the same signature on its own. The situation is similar if one interface has a default method, and in the other the same method is abstract - no class default implementation is inherited.
* The default method cannot override the class method `java.lang.Object`.
* They help implement com.core.example.interfaces without fear of disrupting other classes.
* Avoid creating utility classes, since all the necessary com.core.example.methods can be represented in the com.core.example.interfaces themselves.
* They give classes the freedom to choose the method to be redefined.
* One of the main reasons for introducing default com.core.example.methods is the ability of com.core.example.collections in Java 8 to use lambda expressions.
...
Static interface com.core.example.methods are similar to default com.core.example.methods, except that there is no way to override them in classes that implement the interface.

* Static com.core.example.methods in the interface are part of the interface without the ability to use them for objects of the implementation class
* Class com.core.example.methods `java.lang.Object`cannot be overridden as static
* Static com.core.example.methods in the interface are used to provide helper com.core.example.methods, for example, checking for null, sorting com.core.example.collections, etc.
...
Associative com.core.example.arrays (maps), for example `HashMap`, are not supported.
...
## Q. Why do streams use `forEach()`and `forEachOrdered()` com.core.example.methods?
...
## Q. What are `map()`, `mapToInt()`, `mapToDouble()` and `mapToLong()` com.core.example.methods in Stream?
...
## Q. What streamers designed com.core.example.methods `flatMap()`, `flatMapToInt()`, `flatMapToDouble()`, `flatMapToLong()`?
...
Using, com.core.example.methods `parallel()`and `sequential()`it is possible to determine which operations can be parallel, and which only sequential. You can also make a parallel stream from any sequential stream and vice versa:
```java
collection
  .stream ()
...
## Q. What are the final com.core.example.methods of working with streams you know?

* `findFirst()` returns the first element
* `findAny()` returns any suitable item
* `collect()` presentation of results in the form of com.core.example.collections and other data structures
...
## Q. What intermediate com.core.example.methods of working with streams do you know?
...
Stream 
    ...
    .getAsDouble (); // 13.8
```
## Q. What additional com.core.example.methods for working with associative com.core.example.arrays (maps) appeared in Java 8?
...
`LocalDateTime`combines together `LocaleDate`and `LocalTime`contains the date and time in the calendar system ISO-8601 without reference to the time zone. Time is stored accurate to the nanosecond. It contains many convenient com.core.example.methods such as plusMinutes, plusHours, isAfter, toSecondOfDay, etc.
...
Base64 contains 6 basic com.core.example.methods:
...
## Q. What are the functional com.core.example.interfaces `Function<T,R>`, `DoubleFunction<R>`, `IntFunction<R>` and `LongFunction<R>`?
...
Default com.core.example.methods can be used to build call chains ( `compose`, `andThen`).
```java
...
## Q. What are the functional com.core.example.interfaces `UnaryOperator<T>`, `DoubleUnaryOperator`, `IntUnaryOperator`and `LongUnaryOperator`?
...
## Q. What are the functional com.core.example.interfaces `BinaryOperator<T>`, `DoubleBinaryOperator`, `IntBinaryOperator`and `LongBinaryOperator`?
...
## Q. What are the functional com.core.example.interfaces `Predicate<T>`, `DoublePredicate`, `IntPredicateand` `LongPredicate`?
...
The interface contains a variety of com.core.example.methods by default, allow to build complex conditions ( `and`, `or`, `negate`).
```java
...
## Q. What are the functional com.core.example.interfaces `Consumer<T>`, `DoubleConsumer`, `IntConsumer`and `LongConsumer`?
...
## Q. What are the functional com.core.example.interfaces `Supplier<T>`, `BooleanSupplier`, `DoubleSupplier`, `IntSupplier`and `LongSupplier`?
