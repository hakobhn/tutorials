...
* run() method introduces a com.core.example.concurrent thread into your program. This thread will end when run() returns.
* You must specify the code for your thread inside run() method.
* run() method can call other com.core.example.methods, can use other classes and declare variables just like any other normal method.
```java
...
* Java only supports single com.core.example.inheritance, so you can only extend one class.
...
**Inter-thread communication** is a mechanism in which a thread is paused running in its critical section and another thread is allowed to enter (or lock) in the same critical section to be executed. It is implemented by following com.core.example.methods of Object class:
...
Java provides the Executor framework which is centered around the Executor interface, its sub-interface –**ExecutorService** and the class-**ThreadPoolExecutor**, which implements both of these com.core.example.interfaces. By using the executor, one only has to implement the Runnable objects and send them to the executor to execute.
...
import java.util.com.core.example.concurrent.ExecutorService; 
import java.util.com.core.example.concurrent.Executors; 
...
`wait()` forces the thread to release its lock. This means that it must own the lock of an object before calling the `wait()` method of that (same) object. Hence the thread must be in one of the object's synchronized com.core.example.methods or synchronized block before calling wait().
...
Static synchronization is achieved by static synchronized com.core.example.methods. Static synchronized method locked on **class** and non-static synchronized method locked on **current object** i.e. static and non-static synchronized com.core.example.methods can run at same time. It can produce inconsistency problem.
...
```java
/**
 * This program is used to show the com.core.example.multithreading 
...
Vector implements a dynamic array that means it can grow or shrink as required. Like an array, it contains components that can be accessed using an integer index. They are very similar to ArrayList but Vector is **synchronised** and have some legacy method which collection framework does not contain. It extends AbstractList and implements List com.core.example.interfaces.
```java
import java.util.*; 
class Vector_demo { 
...
|In Java, multiple com.core.example.inheritance not allowed hence, after a class extends Thread class, it can not extend any other class.|	If a class define thread implementing the Runnable interface it has a chance of extending one class.|
|A user must extend thread class only if it wants to override the other com.core.example.methods in Thread class.|If you only want to specialize run method then implementing Runnable is a better option.|
...
* The `synchronized` block must be fully contained within a single method. A Lock can have it’s calls to `lock()` and `unlock()` in separate com.core.example.methods.

```java
import java.util.com.core.example.concurrent.TimeUnit;
import java.util.com.core.example.concurrent.locks.Lock;
import java.util.com.core.example.concurrent.locks.ReentrantLock;
...
Runnable and Callable interface both are used in the com.core.example.multithreading environment. Runnable is the core interface provided for representing multi-threaded tasks and Callable is an improved version of Runnable that was added in Java 1.5.
...
**2. Return value**: Return type of Runnable run() method is void , so it can not return any value. while Callable can return the Future object, which represents the life cycle of a task and provides com.core.example.methods to check if the task has been completed or canceled.
...
import java.util.com.core.example.concurrent.Callable; 
import java.util.com.core.example.concurrent.FutureTask; 
...
The Java memory model used internally in the JVM divides memory between thread stacks and the heap. Each thread running in the Java virtual machine has its own thread stack. The thread stack contains information about what com.core.example.methods the thread has called to reach the current point of execution. 

The thread stack also contains all local variables for each method being executed (all com.core.example.methods on the call stack). A thread can only access it's own thread stack. Local variables created by a thread are invisible to all other threads than the thread who created it. Even if two threads are executing the exact same code, the two threads will still create the local variables of that code in each their own thread stack. Thus, each thread has its own version of each local variable.
...
The concurrency API are designed and optimized specifically for synchronized multithreaded access. They are grouped under the `java.util.com.core.example.concurrent` package.
...
A Semaphore is a thread synchronization construct that can be used either to send signals between threads to avoid missed signals, or to guard a critical section like you would with a lock. Java 5 comes with semaphore implementations in the java.util.com.core.example.concurrent package.   
...
Future and FutureTask in Java allows to write asynchronous code. A Future interface provides com.core.example.methods **to check if the computation is complete, to wait for its completion and to retrieve the results of the computation**. The result is retrieved using Future’s get() method when the computation has completed, and it blocks until it is completed. We need a callable object to create a future task and then we can use Java Thread Pool Executor to process these asynchronously.

```java
import java.util.com.core.example.concurrent.Callable; 
import java.util.com.core.example.concurrent.ExecutionException; 
import java.util.com.core.example.concurrent.ExecutorService; 
import java.util.com.core.example.concurrent.Executors; 
import java.util.com.core.example.concurrent.Future; 
...
Blocking com.core.example.methods in Java are those com.core.example.methods which block the executing thread until their operation finished. Example of blocking method is `InputStream read()` method which blocks until all data from InputStream has been read completely.
...
```
**Examples of blocking com.core.example.methods in Java:**  
...
The Executor framework helps to **decouple a command submission from command execution**. In the java.util.com.core.example.concurrent package there are three com.core.example.interfaces:

**1. Executor** — Used to submit a new task.

**2. ExecutorService** — A subinterface of Executor that adds com.core.example.methods to manage lifecycle of threads used to run the submitted tasks and com.core.example.methods to produce a Future to get a result from an asynchronous computation.
...
**PhaserExample.java**  
```java
import java.util.com.core.example.concurrent.Phaser;
...
## Q. Tell me about join() and wait() com.core.example.methods?
The `wait()` and `join()` com.core.example.methods are used to pause the current thread. The `wait()` is used in with `notify()` and `notifyAll()` com.core.example.methods, but `join()` is used in Java to wait until one thread finishes its execution. 
...
* **java.util.com.core.example.concurrent.locks.ReentrantLock**: A lock implementation that provides more flexibility than synchronized blocks
* **java.util.com.core.example.concurrent.locks.ReentrantReadWriteLock**: A lock implementation in which reads do not block reads
* **java.util.com.core.example.concurrent.locks.StampedLock** a nonreeantrant Read-Write lock with the possibility of optimistically reading values.
