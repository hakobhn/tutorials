...
Most of the classes and com.core.example.interfaces required to create servlets are contained in `javax.servlet` and packages `javax.servlet.http`.

**Basic servlet com.core.example.methods**
...
* Servlets provide better query processing performance and more efficient use of memory by taking advantage of com.core.example.multithreading (a new thread is created for each request, which is faster than allocating memory for a new object for each request, as happens in CGI).
...
## Q. How does the servlet container manage the servlet life cycle, when and what com.core.example.methods are called?
...
```
Then create a class `xyz.company.ExampleServlet` by inheriting from `HttpServlet` and implement the logic of its work in the method `service()`/ com.core.example.methods `doGet()`/ `doPost()`.
...
* **Servlet and resource lifecycle management**: From servlet loading into memory, initialization, implementation of com.core.example.methods, and ending with servlet destruction. The container also provides additional utilities, such as JNDI, for managing the resource pool.
...
Two com.core.example.methods are implemented in the interface:
...
## Q. What are the differences between forward() and sendRedirect() com.core.example.methods?
**forward()**
...
The web application has the ability to work with attributes using the com.core.example.methods `setAttribute()`, `getAttribute()`, `removeAttribute()`, `getAttributeNames()`, who provided com.core.example.interfaces ServletRequest, HttpSessionand ServletContext(for the scope request, the session, context The respectively).
...
Custom handlers can `ServletRequest` also `ServletResponse` be implemented by adding new or overriding existing com.core.example.methods for wrapper classes `ServletRequestWrapper( HttpServletRequestWrapper)` and `ServletResponseWrapper( HttpServletRequestWrapper)`.
...
## Q. What are the main com.core.example.methods present in the class HttpServlet?
...
An HTTP method is called immutable if it always returns the same result on the same request. HTTP com.core.example.methods `GET`, `PUT`, `DELETE`, `HEAD` and `OPTIONS` are immutable, so it is necessary to implement an application so that these com.core.example.methods return the same results consistently. Variable com.core.example.methods include a method `POST` that is used to implement something that changes with each request.
...
## Q. What are the different com.core.example.methods for managing session in servlets?
...
* **Session Management API** - Using a special API for session tracking, built on the basis and on the com.core.example.methods described above and which solves particular problems of the listed com.core.example.methods:
    ...
    * All of the above com.core.example.methods are not universal: for each of them you can choose a specific scenario in which they will not work.

<div align="right">
    <b><a href="#">↥ back to top</a></b>
</div>

## Q. What are cookies? What com.core.example.methods for working with cookies are provided in servlets?
...
* To get an array of cookies from the request, you must use the method `HttpServletRequest.getCookies()`. There are no com.core.example.methods for adding cookies to `HttpServletRequest`.
...
To be sure that the object will be notified about the termination of the session, you need to implement the interface `javax.servlet.http.HttpSessionBindingListener`. Two com.core.example.methods of this interface: `valueBound()` and `valueUnbound()` are used when adding an object as an attribute to a session and when destroying a session, respectively.
...
## Q. What authentication com.core.example.methods are available to the servlet?
...
A servlet filter can be configured to work with a single servlet or a group of servlets. The basis for the formation of filters is an interface javax.servlet.Filterthat implements three com.core.example.methods:
